...
func TestWrapClient_RoutesWatchThroughWatchFunc(t *testing.T) {
	...
	var (
		...
		gotNS  string
		gotGet k8sinformer.GetFunc
		calls  int
	)
	fn := func(_ context.Context, gvr schema.GroupVersionResource, namespace string, get k8sinformer.GetFunc, _ metav1.ListOptions) (watch.Interface, error) {
		calls++
...
func TestWrapClient_DelegatesListToReal(t *testing.T) {
	...
	fn := func(context.Context, schema.GroupVersionResource, string, k8sinformer.GetFunc, metav1.ListOptions) (watch.Interface, error) {
		watchCalled = true
		return nil, nil
	}
