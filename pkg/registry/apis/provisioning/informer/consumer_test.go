...
func (f *fakeSubscriber) publish(t *testing.T, subject string, evt resourcewatch.Event) {
	t.Helper()
...
func repoEvent(action resourcewatch.EventType, name string, rv int64) resourcewatch.Event {
	gvr := provisioningapis.RepositoryResourceInfo.GroupVersionResource()
	return resourcewatch.Event{
		Type:            action,
		Group:           gvr.Group,
...
func TestConsumer_MaterializesAddModifyViaGet(t *testing.T) {
	...
	sub.publish(t, repoSubject(), repoEvent(resourcewatch.Added, "repo-a", 1))
...
func TestConsumer_DeletedObjectDefersToRelist(t *testing.T) {
	...
	sub.publish(t, repoSubject(), repoEvent(resourcewatch.Deleted, "repo-a", 9))
	requireNoEvent(t, w)
}
...
func TestConsumer_SkipsVanishedObject(t *testing.T) {
	...
	sub.publish(t, repoSubject(), repoEvent(resourcewatch.Added, "ghost", 1)) // 404 -> skipped
	sub.publish(t, repoSubject(), repoEvent(resourcewatch.Modified, "real", 5))
...
func TestConsumer_SkipsMalformedAndUnknown(t *testing.T) {
	...
	// Unknown verb.
	sub.publish(t, repoSubject(), repoEvent("BOGUS", "x", 1))

	// A valid notification after the bad ones must still arrive — bad
	// notifications are skipped, not fatal to the watch.
	sub.publish(t, repoSubject(), repoEvent(resourcewatch.Added, "survivor", 7))
	repo := nextRepo(t, w, watch.Added)
...
func TestConsumer_RelistsAfterInterval(t *testing.T) {
	c := NewConsumer(newFakeSubscriber())
	c.relistInterval = 200 * time.Millisecond
