...
// Decode returns the next watch event. Every notification carries only metadata,
// so the decoder fetches the current object with Get and delivers that. It skips
// notifications it cannot turn into a faithful event — malformed, unknown verb,
// or an object that has already vanished — rather than tearing the watch down,
// since one bad notification must not stop delivery of the rest.
//
...
func (d *decoder) Decode() (watch.EventType, runtime.Object, error) {
	for {
		...
		switch evt.Type {
		case resourcewatch.Added, resourcewatch.Modified, resourcewatch.Deleted:
		default:
		...
		// The object exists, so deliver its current state. A DELETED
		// notification whose object is still present (finalization in progress,
		// or a recreate) is delivered as MODIFIED — delivering a delete would
		// evict a live object from the cache.
		action := watch.EventType(evt.Type)
